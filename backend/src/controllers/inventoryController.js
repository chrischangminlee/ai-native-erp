  ...
  try {
    ...
    res.json({
      kpis: {
        ...
        out_of_stock: {
          value: stockAnalysis.out_of_stock || 0,
...
export async function getByItem(req, res) {
  try {
  ...
  try {
    ...
    res.json({
      ...
      summary: {
        ...
        products_affected: valueAtRisk.products_affected || 0,
        lots_affected: valueAtRisk.lots_affected || 0,
...
export async function getValuation(req, res) {
  try {
    ...
    const inventoryToSalesRatio = monthlySales.avg_monthly_sales > 0
      ? (inventoryValue.total_value / monthlySales.avg_monthly_sales).toFixed(2)
      : 0;

    // Valuation comparison
    const totalFifo = currentValuation.reduce((sum, cat) => sum + cat.fifo_value, 0);
    const totalAverage = currentValuation.reduce((sum, cat) => sum + cat.average_value, 0);
    const totalLifo = currentValuation.reduce((sum, cat) => sum + cat.lifo_value, 0);

    res.json({
      currentValuation: currentValuation.map(cat => ({
      ...
      metrics: {
        ...
        inventory_to_sales_ratio: parseFloat(inventoryToSalesRatio),
        method_used: method
      }
    });
  } catch (error) {
